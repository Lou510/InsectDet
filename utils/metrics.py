...
def shape_power_iou(box1, box2, Lambda=1.3, eps=1e-7, xywh=True, scale=0):
    (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
    w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
    b1_x1, b1_x2, b1_y1, b1_y2 = x1 - w1_, x1 + w1_, y1 - h1_, y1 + h1_
    b2_x1, b2_x2, b2_y1, b2_y2 = x2 - w2_, x2 + w2_, y2 - h2_, y2 + h2_
    ...
    iou = 1-iou + distance + 0.5 * (shape_cost)
    # iou = 1-iou + distance
    # iou=1 - iou
    q_shape = torch.exp(-distance)
    # q_shape = torch.exp(-(distance + 0.5 * (shape_cost)))
    x_shape=q_shape*Lambda

    return 1 - 3*x_shape*torch.exp(-x_shape**2)*iou  # IoU
