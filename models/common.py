...
class DMCA(nn.Module):
    def __init__(self, dim, num_heads=8, LayerNorm_type='WithBias'):
        ...
        # Dynamic sampling ratio (learnable parameter)
        self.dynamic_ratio = nn.Sequential(
    ...
    def weighted_sample(self, x, ratio, keep_center=True, target_dim=None):
        b, head, seq, dim = x.shape
        num_samples = max(1, int(seq * ratio))
        indices = torch.randperm(seq, device=x.device)[:num_samples]

        if keep_center:
            center_idx = seq // 2
            indices = torch.cat([indices, torch.tensor([center_idx], device=x.device)]).unique()

        x_sampled = x[:, :, indices, :]

        if target_dim is not None:
            pad_len = target_dim - x_sampled.shape[2]
            if pad_len > 0:
                x_sampled = F.pad(x_sampled, (0, 0, 0, pad_len))  # 在序列维度填充
    ...
    def forward(self, x):
        ...
        ratio = self.dynamic_ratio(x).view(b, 1, 1, 1).mean().item()

        q1, indices = self.weighted_sample(q1, ratio, target_dim=h)
        k1 = k1[:, :, indices, :]
        v1 = v1[:, :, indices, :]

        q2, indices = self.weighted_sample(q2, ratio, target_dim=w)
        k2 = k2[:, :, indices, :]
        v2 = v2[:, :, indices, :]
        ...
        h_sampled, w_sampled = q1.shape[2], q2.shape[2]

        out3 = rearrange(out3, 'b head h (w c) -> b (head c) h w', head=self.num_heads, h=h_sampled, w=w_sampled)
        out4 = rearrange(out4, 'b head w (h c) -> b (head c) h w', head=self.num_heads, h=h_sampled, w=w_sampled)
